...
void contatore(int vet[];int min;int max)
{
	...
	printf("numeri tra ",min,"e ",max,":%d\n",conta);
}
int contatore(int vet[],int min,int max);
int main(int argc,char *argv[])
{
	...
	{
		int array[dim],num,kont,conta[2];
		...
		contatore(array[],1,10));
		contatore(array[],30,40));
		contatore(array[],5,50));
		return 0;
	}
}
