	...
	while (1) {
		...
		if (FD_ISSET(1, &rset)) {
			...
			if (strcmp(intBuff, "\n")) {
				...
				case 104:
					...
					break;
				case 105:
					/* Selecting to bid one item */
					sprintf(conv, "%d", client_id);
					if (!strcpy(buff, conv))
						return -1;
					if (!strcat(buff, "_"))
						return -1;
					if (!strcat(buff, BID_ITEM))
						return -1;
		...
		if (FD_ISSET(sockfd, &rset)) {
			...
			if (n == 0) {
				...
				if (strstr(buff, MENU))
					action = 101;
				// Insert the id of the item you want to bid
				else if (strstr(buff, BID)) {
					action = 105;
					item_id = parseItemIdClient(buff); //buff[strlen(buff) - 2];
				}
